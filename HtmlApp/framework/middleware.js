...
var httpReqHandleWare = {
  ...
  handleRequestParam: function(req, callback) {
    ...
    if ("GET" == req.method) {
      let urlObj = url.parse(req.url);
      let queryObj = querystring.parse(querystring.unescape(urlObj.query));
      ...
      req.on("end", () => {
        ...
        let queryObj = querystring.parse(querystring.unescape(content));
        param = Object.assign({}, queryObj);
        req["params"] = param;
        console.log(param)
        if (!!callback) {
...
var httpHandlerWare = {
  ...
  busiHandle: function(executor, req, res) {
    try {
      ...
      return this.exceptionHandler(err, req, res);
    }
  },
